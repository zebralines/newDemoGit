    ...
    bindDesc:function()
    {
      ...
      var parms = {
          ...
          callback:function(ret){
              ...
              {
                //小图轮番
                var smalllist =  ret.data.small_images;
                smalllist = smalllist.split("|");
                for (var i = 0; i < smalllist.length; i++) {
                    var img_src = smalllist[i];
                    var img_src_path = img_src+"_350x350";
                  ...
                  //  }
                    ...
                    pageMol.topimags.push(img_src_path);
                }

                //设置店铺相关
                var shopIcon = ret.data.shopIcon;
                //店铺图片
                $api.attr($api.dom(".store_logo"),'src',ret.data.shopIcon);

                //店铺标题
                $api.text($api.dom("#shoptitle"),ret.data.shop_title);
                //判断天猫 还是淘宝
                if(ret.data.user_type == 0)
                {
                 ...
                 //商品详细描述
                  var imglist = ret.data.pcDescContent;
                  if(imglist != ""){
                    imglist = imglist.split("|");
                    var htmlstr = "";
                    for (var i = 0; i < imglist.length; i++) {
                      var urlimg = imglist[i];
                      var imgs = "http://"+urlimg;
